...
RUN PATIENTAPI_URL=$(curl -sL "${RELEASE_URL}" | jq -r '.assets[] | select(.name == "PatientAPI.jar") | .url') && \
    curl -sLJO -H "Accept: application/octet-stream" "${PATIENTAPI_URL}"


RUN cp PatientAPI.jar /home/ballerina/PatientAPI.jar
